...
{
  if (selfAllocated) delete[] frameBuffer;
}

void LEDMatrixDriver::setPixel(int16_t x, int16_t y, bool enabled)
{
  uint8_t* p = _getBufferPtr(x, (modRev) ? (7 ^ y) : y);    // <============= Changed 7 - y : y
  if (!p) return;

  uint16_t b = (modRev) ? (x & 7) : (7 ^ (x & 7));   //bit  // <============= Changed 7 - (x & 7)
...
void LEDMatrixDriver::_displayRow(uint8_t row)
{
  ...
  {
    uint16_t cmd = ((row + 1) << 8) | frameBuffer[i + row * N];
    SPI.transfer16(cmd);
  }
  digitalWrite(ssPin, 1);
  SPI.endTransaction();
...
{
  if (y >= 8     || y < 0) return nullptr;
  if (x >= 8 * N || x < 0) return nullptr;
...
{
  for (uint8_t y = 0; y < 8; y++) _displayRow(y);
}
...
void LEDMatrixDriver::scroll( scrollDirection direction )
{
  ...
  switch ( direction )
  {
    case scrollDirection::scrollUp:
      i = 7 * N; // moving 7 rows of N segments
      if (modRev)
      {
        memmove(frameBuffer + N, frameBuffer, i);
        memset(frameBuffer, 0, N);          // Clear first row
      }
      else
      {
        memmove(frameBuffer, frameBuffer + N, i);
        memset(frameBuffer + i, 0, N);      // Clear last row
      }
      /*      // <============= Changed
    ...
    case scrollDirection::scrollRight:
      ...
      for (i = 8 * N - 1; i >= 0; i--)  // <============= Changed !!!!!! corrupting memory !!!!!!
      {
        if (modRev)
        {
          uint8_t n = frameBuffer[i] & 0x80;
          frameBuffer[i] <<= 1;
          if ((i + 1) % N) frameBuffer[i + 1] |= (n >> 7);
        }
        else
        {
          uint8_t n = frameBuffer[i] & 0x01;
          frameBuffer[i] >>= 1;
          if ((i + 1) % N) frameBuffer[i + 1] |= (n << 7);
        }
      }
