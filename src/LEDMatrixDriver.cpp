...
void LEDMatrixDriver::scroll( scrollDirection direction )
{
	...
	switch( direction )
	{
      ...
      for (cnt = 8 * N; cnt >= 0; cnt--)
      {
        if (modRev)
        {
          uint8_t n = frameBuffer[cnt] & 0x80;
          frameBuffer[cnt] <<= 1;
          if ((cnt + 1) % N) frameBuffer[cnt + 1] |= (n >> 7);
        }
        else
        {
          uint8_t n = frameBuffer[cnt] & 0x01;
          frameBuffer[cnt] >>= 1;
          if ((cnt + 1) % N) frameBuffer[cnt + 1] |= (n << 7);
        }
      }
      ...
      for (cnt = 0; cnt < 8 * N; cnt++)
      {
        if (modRev)
        {
          uint8_t n = frameBuffer[cnt] & 0x01;
          frameBuffer[cnt] >>= 1;
          if (cnt % N) frameBuffer[cnt - 1] |= (n << 7);
        }
        else
        {
          uint8_t n = frameBuffer[cnt] & 0x80;
